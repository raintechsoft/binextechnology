...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h1 class="text-3xl font-extrabold text-white tracking-tight">Edit Client</h1>
        <p class="text-slate-400 text-sm mt-2">Update details for {{ $client->name }}.</p>
    </div>
...
<div class="bg-slate-900 rounded-2xl shadow-xl shadow-black/20 border border-slate-800 p-6 md:p-8 max-w-3xl relative overflow-hidden">
    ...
    <form action="{{ route('admin.clients.update', $client) }}" method="POST" enctype="multipart/form-data" class="relative z-10">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <div class="mb-4">
                ...
                <div class="p-4 border border-slate-700 rounded-xl bg-slate-800/50 inline-block shadow-inner">
                    <img src="{{ Storage::url($client->logo) }}" alt="Current Image" class="h-20 w-auto object-contain bg-white rounded p-1">
                </div>
            </div>
            @endif
            ...
            <div class="mt-1 border-2 border-slate-700 border-dashed rounded-xl px-6 pt-5 pb-6 flex justify-center hover:bg-slate-800/50 hover:border-slate-600 transition-all group cursor-pointer" onclick="document.getElementById('logo').click()">
                <div class="space-y-1 text-center">
                    <i class="fas fa-image text-4xl text-slate-500 mb-2 group-hover:text-blue-400 transition-colors"></i>
            ...
            <div id="imagePreviewContainer" class="mt-4 hidden">
                <p class="text-sm font-medium text-slate-400 mb-2">New Image Preview:</p>
        ...
        <div class="mb-8">
            <label class="flex items-center cursor-pointer group">
                 <div class="relative flex items-center justify-center">
                    <input type="checkbox" name="is_active" value="1" class="peer sr-only" {{ old('is_active', $client->is_active) ? 'checked' : '' }}>
