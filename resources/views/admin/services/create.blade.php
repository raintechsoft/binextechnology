...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h1 class="text-3xl font-extrabold text-white tracking-tight">Add New Service</h1>
        <p class="text-slate-400 text-sm mt-2">Create a new service offering for your website.</p>
    </div>
    ...
    <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data" class="relative z-10">
            ...
            <div class="mt-1 border-2 border-slate-700 border-dashed rounded-xl px-6 pt-5 pb-6 flex justify-center hover:bg-slate-800/50 hover:border-slate-600 transition-all group cursor-pointer" onclick="document.getElementById('image').click()">
                <div class="space-y-1 text-center">
                    ...
                    <div class="flex flex-col sm:flex-row text-sm text-slate-400 justify-center gap-1">
                        <label for="image" class="relative cursor-pointer rounded-md font-medium text-blue-400 hover:text-blue-300 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500 focus-within:ring-offset-slate-900">
        ...
        <div class="mb-6">
            ...
            <textarea id="short_description" name="short_description" rows="3" required
                class="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 text-white transition-all placeholder-slate-500">{{ old('short_description') }}</textarea>
