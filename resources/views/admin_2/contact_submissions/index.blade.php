...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-100">Contacts / Social Media</h1>
        <p class="text-gray-400 text-sm mt-1">View contacts and social media submissions through the website.</p>
    </div>
        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100 text-gray-200">
                ...
                <tr class="hover:bg-gray-900 transition-colors">
                    <td class="px-6 py-4 font-medium text-white">{{ $submission->name }}</td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col text-sm">
                            ...
                            @if($submission->phone)
                            <span class="text-gray-300 mt-1"><i class="fas fa-phone mr-1 text-gray-400"></i> {{ $submission->phone }}</span>
                            @endif
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        ...
                        <p class="text-xs text-gray-400 mt-1 truncate max-w-xs" title="{{ $submission->message }}">{{ Str::limit($submission->message, 30) }}</p>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('admin_2.contact_submissions.edit', $submission) }}" class="text-blue-600 hover:text-blue-800 p-2 border border-blue-200 hover:bg-blue-50 bg-gray-800 rounded transition-colors" title="Edit">
